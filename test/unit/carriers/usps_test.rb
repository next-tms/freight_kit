...
class USPSTest < ActiveSupport::TestCase
  ...
  def test_find_tracking_info_should_return_a_tracking_response
    ...
    assert_instance_of Interstellar::TrackingResponse, @carrier.find_tracking_info('9102901000462189604217', :test => true)
    @carrier.expects(:commit).returns(@tracking_response)
    assert_equal 'Interstellar::TrackingResponse', @carrier.find_tracking_info('EJ958083578US').class.name
  end
  ...
  def test_batch_find_tracking_info_should_return_a_tracking_response_array
    ...
    assert responses.all? { |x| x.instance_of? Interstellar::TrackingResponse}
  end
