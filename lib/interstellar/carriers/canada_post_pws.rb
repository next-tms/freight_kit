module Interstellar
  class CanadaPostPWS < Carrier
    ...
    rescue ActiveUtils::ResponseError, Interstellar::ResponseError => e
    ...
    def parse_rates_response(response, origin, destination, exclude_tax)
      doc = Nokogiri.XML(response)
      doc.remove_namespaces!
      raise Interstellar::ResponseError, "No Quotes" unless doc.at('price-quotes')
    ...
    def parse_tracking_response(response)
      doc = Nokogiri.XML(response)
      doc.remove_namespaces!
      raise Interstellar::ResponseError, "No Tracking" unless doc.at('tracking-detail')
    ...
    def parse_shipment_response(response)
      doc = Nokogiri.XML(response)
      doc.remove_namespaces!
      raise Interstellar::ResponseError, "No Shipping" unless doc.at('non-contract-shipment-info')
      options = {
    ...
    def parse_register_token_response(response)
      doc = Nokogiri.XML(response)
      doc.remove_namespaces!
      raise Interstellar::ResponseError, "No Registration Token" unless doc.at('token')
      options = {
