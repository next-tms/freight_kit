  ...
  class FedEx < Carrier
    ...
    def build_request_header(xml)
      ...
      xml.TransactionDetail do
        xml.CustomerTransactionId(@options[:transaction_id] || 'HyperCarrier') # TODO: Need to do something better with this...
      end
    end
    ...
    def build_document(xml, expected_root_tag)
      ...
      if document.root.nil? || document.root.name != expected_root_tag
        raise HyperCarrier::ResponseContentError.new(StandardError.new('Invalid document'), xml)
      end
      document
    rescue Nokogiri::XML::SyntaxError => e
      raise HyperCarrier::ResponseContentError.new(e, xml)
    end
